...
const Paidorderlistdetail = (props) => {
  var node = useRef()
  ...
  var { loading, data } = useQuery(GET_EXCEL, {
    variables: {
      adminId: Number(props.match.params.id),
    },
    onCompleted: (data) => {
  ...
  // const [getexcelbyid, { data223, error2231 }] = useMutation(GET_EXCEL, {
  //   onCompleted: (data223) => {
  //     console.log('excelDatabyadminid', data223)
  //     // if (data223.excelDatabyadminid[0].datasource) {
  //     //   setexceldatasource(JSON.parse(data223.excelDatabyadminid[0].datasource))
  //     // }
  //   },
  //   onError: (error2231) => {
  //     console.log('err', error2231)
  //   },
  // })

  const [getuserbyid, { data22, error9 }] = useMutation(GET_USERBYID, {
    onCompleted: (data22) => {
      setFormState({
        ...formState,
        adminuserinfo: data22.userbyid,
      })
      // getexcelbyid({
      //   variables: {
      //     adminId: Number(props.match.params.id),
            ...
            <Card>
              <CardHeader>
                ...
                <h5>
                  {' '}
                  사업자 명 -{' '}
                  {formState.adminuserinfo?.company?.split('///')[0]}
                </h5>
                <h5>
                  {' '}
                  사업자 주소 -{' '}
                  {formState.adminuserinfo?.company?.split('///')[1]}
                </h5>
              </CardHeader>
              <CardBody>
                <div
...
export default Paidorderlistdetail
