...
`
const GET_USERS = gql`
  query commonusersbyadmin($adminid: Int) {
    commonusersbyadmin(adminid: $adminid) {
      id
      createdAt
      name
      email
...
const Userlist = () => {
  ...
  var { data: data1 } = useQuery(GET_USERS, {
    ...
    onCompleted: (data1) => {
      setFormState({
        ...formState,
        users: data1.commonusersbyadmin,
      })
    },
    onError: (error) => {
      console.log('err', error)
  ...
  const { meerror, data: datame } = useQuery(ME_QUERY, {
    onCompleted: (datame) => {
