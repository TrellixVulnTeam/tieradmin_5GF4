...
const Query_Reservadmin = gql`
  query reservationbyadminall {
    reservationbyadminall {
      userId
      date
      time
      id
...
const Orderlisttotal = () => {
  ...
  const { error: error212, data: data2121 } = useQuery(Query_Reservadmin, {
    onCompleted: (data2121) => {
      console.log('query com', data2121.reservationbyadminall)
      sestresultdata(data2121.reservationbyadminall)
    },
    onError: (error212) => {
